...
class WpNavi():
    def __init__(self):  # コンストラクタ
        ...
        rospy.Subscriber('buttun_pushed', Bool, self.buttun_pushed)
        ...
        # [x,y,theta]
        way_point = [[ 6.1, 0.8, 0.0 * pi], [6.1, 4.2, 0.5 * pi],
                     [ 2.7, 4.2, -1.0 * pi], [2.7, 0.8, -0.5 * pi],
                     [999, 999, 999]]

        self.i = 0

    def buttun_pushed(self, buttun_pushed):
        if buttun_pushed == True:
            # ROSではロボットの進行方向がx座標、左方向がy座標、上方向がz座標
            self.goal.target_pose.pose.position.x = way_point[self.i][0]
            self.goal.target_pose.pose.position.y = way_point[self.i][1]
        if way_point[self.i][0] == 999:
                break

        q = tf.transformations.quaternion_from_euler(0, 0, way_point[self.i][2])
